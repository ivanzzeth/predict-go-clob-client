...
func TestDoRequest_JWTRefreshOn401_AuthorizationError(t *testing.T) {
	var requestCount atomic.Int32
	var authMessageCount atomic.Int32
	var authCount atomic.Int32
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/auth/message":
			...
			resp := types.APIBaseResponse[types.AuthMessageResponse]{
				Success: true,
				Data:    types.AuthMessageResponse{Message: "test message"},
			}
			json.NewEncoder(w).Encode(resp)
		case "/v1/auth":
			authCount.Add(1)
			resp := types.APIBaseResponse[types.JWTTokenResponse]{
				Success: true,
				Data:    types.JWTTokenResponse{Token: "new-jwt-token"},
			}
			json.NewEncoder(w).Encode(resp)
		case "/v1/test":
			count := requestCount.Add(1)
			if count == 1 {
				w.WriteHeader(http.StatusUnauthorized)
				w.Write([]byte(`{"success":false,"code":401,"error":"unauthorized","message":"authorization error"}`))
				return
			}
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`{"success":true,"data":{}}`))
	...
	_, err := client.doRequest("GET", "/v1/test", nil, true)
	if err != nil {
		t.Fatalf("Expected success after JWT refresh on generic 401, got error: %v", err)
	}
